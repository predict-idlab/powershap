...
def deepSHAP(
    ...
    for i in tqdm(range(loop_its)):
        ...
        tf.random.set_seed(i+random_seed_start)
        ...
        _ = PowerSHAP_model.fit(X_train_feat,Y_train, batch_size=batch_size, epochs=epochs, validation_data=(X_val_feat,Y_val),verbose=False)

        C_explainer = shap.DeepExplainer(PowerSHAP_model,X_train_feat)

        Shap_values = C_explainer.shap_values(X_val_feat)[0]
