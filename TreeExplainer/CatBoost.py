...
def PowerSHAP(
    ...
    for i in range(len(shaps_pd.columns)):
        quantile = (
            base_functions.p_values_arg_coef(
                np.array(shaps_pd.values[:, i]), mean_random_uniform
            )
            / 100
        )
        p_value = quantile
        ...
        if include_all or p_value < power_alpha:
            pooled_standard_deviation = np.sqrt(
                (
                    (shaps_pd.std()[i] ** 2) * (n_samples - 1)
                    + (n_samples - 1)
                    * (shaps_pd["random_uniform_feature"].values.std() ** 2)
                )
                / (n_samples * 2 - 2)
            )
            effect_size.append(
                (np.abs(shaps_pd.mean()[i] - mean_random_uniform))
    ...
    processed_shaps_pd = pd.DataFrame(
        data=np.hstack(
            [
                np.reshape(shaps_pd.mean().values, (-1, 1)),
        ...
        index=shaps_pd.mean().index,
    )
    processed_shaps_pd = processed_shaps_pd.reindex(
        processed_shaps_pd.Impact.abs().sort_values(ascending=False).index
    )

    if automatic:
        max_iterations = int(
            np.ceil(
                processed_shaps_pd[processed_shaps_pd.p_value < 0.01][
                    "95_power_its_req"
                ].max()
            )
        )
    ...
    if include_all:
        ...
        return processed_shaps_pd[
            processed_shaps_pd.p_value < power_alpha
        ]  # ,shaps_pd,Shap_values_ar,X_Shap_ar
