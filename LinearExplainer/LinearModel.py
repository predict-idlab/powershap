...
def linearSHAP(
    ...
    for i in tqdm(range(loop_its)):
        ...
        PowerSHAP_model = clone(model).set_params(random_state=i+random_seed_start)
        PowerSHAP_model.fit(X_train_feat, Y_train)

        C_explainer = shap.explainers.Linear(PowerSHAP_model,X_train_feat)
